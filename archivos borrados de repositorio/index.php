...
<body>
    ...
    let currentUserCedula = null; // Variable para guardar la cédula del usuario logueado

    /**
     * Función para mostrar una página específica por su ID y ocultar las demás.
     * También actualiza el hash en la URL para la navegación.
     * @param {string} pageId - El ID del div de la página a mostrar (ej. 'login-page').
     */
    ...
    /**
     * Lógica de navegación principal. Escucha los clics en los enlaces <a>
     * y determina qué página mostrar o si debe realizar una acción como logout.
     */
    document.body.addEventListener('click', function(event) {
        const target = event.target.closest('a'); // Asegura que se captura el enlace
        if (target) {
            ...
            if (href && href.startsWith('#')) {
                ...
                if (targetHash === 'logout') {
                    // Acción de cerrar sesión (simulada por ahora)
    ...
    /**
     * Control de eventos para el formulario de inicio de sesión.
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(event) {
            ...
            // Llamada al backend con Fetch API
            ...
            .then(data => {
                if (data.success) {
                    ...
                    currentUserCedula = formData.get('username'); // Guarda la cédula
                    ...
                    // Si el backend reporta un error
            ...
            .catch(error => {
                ...
                errorMessageDiv.textContent = 'Error de conexión. Intente más tarde.';
                errorMessageDiv.style.display = 'block';
            });
        });
    }
    ...
    /*
    * NOTA: Los siguientes formularios son simulados.
    * Deberás aplicar la misma lógica de `fetch` que se usó en el login
    * para conectarlos a sus respectivos archivos PHP en el backend.
    */
    ...
    if (registroForm) {
        registroForm.addEventListener('submit', function(event) {
            ...
            // Aquí iría la llamada fetch a 'backend/registro.php'
            showPage('login-page');
            this.reset();
        });
    }
    ...
    if (recuperarForm) {
        recuperarForm.addEventListener('submit', function(event) {
            event.preventDefault();
            alert('Funcionalidad de recuperación en construcción.');
            // Aquí iría la llamada fetch a 'backend/recuperar.php'
            showPage('login-page');
            this.reset();
        });
    }
    
    // --- LÓGICA PARA FORMULARIOS DINÁMICOS (Ej. Registro de Novedades) ---
    // Esta parte sigue siendo una simulación y puede ser conectada al backend más adelante.
    ...
    const formsHtml = {
        'ausencia': `<h3>Formulario de Ausencia (Simulado)</h3><p>Contenido del formulario...</p><button>Enviar</button>`,
        'incapacidad': `<h3>Formulario de Incapacidad (Simulado)</h3><p>Contenido del formulario...</p><button>Enviar</button>`,
        // Añade el resto de los HTML de tus formularios aquí...
    };
     ...
     * Lee el hash de la URL al cargar la página para mostrar la vista correcta.
     */
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (initialHash && validPageHashes.includes(initialHash)) {
            // Esto es para permitir cargar una página directamente desde la URL
            // pero por seguridad, si no hay sesión activa, debería redirigir a login.
            // Por ahora, lo dejamos así para pruebas.
